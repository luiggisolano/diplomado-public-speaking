...
/*
  Nube sónica portada a la dirección "Fusión". Reutiliza el fragment shader de bandas
  con domain warping de la dirección Sónica (SONICA_BACKDROP_FRAGMENT) y el andamiaje
  fullscreen compartido, pero se monta detrás de la maquetación de Cámara Anecoica. El
  shader reacciona a la VELOCIDAD del scroll y al puntero; publica la señal suavizada en
  --fus-vel por si algún elemento quiere modularse con ella. Autoprotegido: sin WebGL o
  con reduced-motion no monta nada y deja ver el degradado estático de respaldo del CSS.
*/
...
export function FusionCloud() {
  ...
  useEffect(() => {
    ...
    const handle = mountFullscreenShader({
      canvas,
      fragment: FUSION_CLOUD_FRAGMENT,
      dprCap: 1.75,
      uniforms: {
  ...
  return <canvas ref={canvasRef} className="fus-backdrop__canvas" aria-hidden="true" />;
}
