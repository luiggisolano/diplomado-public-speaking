...
/*
  Firma de autor LS para "La Síntesis".
  Cierra la pieza con el invariante de marca: monograma draw-on-scroll en tinta
  dorada + metadatos en mono. El efecto de dibujo del trazo respeta
  prefers-reduced-motion mostrando la firma completa desde el inicio.
*/
...
export function SynAuthorSignature() {
  ...
  useEffect(() => {
    ...
    const strokes   = Array.from(root.querySelectorAll<SVGPathElement>("[data-syn-sig-stroke]"));
    const splatters = Array.from(root.querySelectorAll<SVGElement>("[data-syn-sig-splatter]"));
  ...
  useEffect(() => {
    const MONOGRAM_STYLE = "color:#ffa902;font-size:15px;font-weight:800;letter-spacing:0.18em;";
    const NOTE_STYLE     = "color:#aebcd4;font-size:11px;letter-spacing:0.04em;";
    console.log(
  ...
  return (
    <footer
      ref={rootRef}
      className="syn-author-footer py-16"
    >
      <div className="mx-auto flex max-w-3xl flex-col items-center gap-5 px-6 text-center">
        ...
        >
          <defs>
            <filter id="syn-ink-texture" x="-10%" y="-10%" width="120%" height="120%">
              <feTurbulence
                type="fractalNoise"
                baseFrequency="0.9"
                numOctaves={1}
          ...
          <g fill="currentColor" stroke="none">
            <circle data-syn-sig-splatter cx={198} cy={64} r={2.1} />
            <circle data-syn-sig-splatter cx={207} cy={73} r={1.4} />
            <circle data-syn-sig-splatter cx={214} cy={59} r={1} />
            <circle data-syn-sig-splatter cx={189} cy={82} r={1.6} />
          </g>
        </svg>
